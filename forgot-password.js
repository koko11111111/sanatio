...
function runForgotPassword() {
  ...
  async function checkEmailServerStatus() {
    ...
    try {
      ...
      if (res.ok) {
        emailjsHint.innerHTML = 'Email server is running. Use <code>start-all.bat</code> to start site + email together.';
        emailjsHint.className = "footer-text reset-emailjs-hint reset-emailjs-hint-ok";
      }
    } catch {
      emailjsHint.innerHTML = 'Start the email server: double-click <code>start-email.bat</code> (or <code>start-all.bat</code>), then try again.';
  ...
  async function sendViaLocalServer(email, userName, code) {
    ...
    if (data.devConsole) console.info("Dev mode: check start-email.bat window for your 6-digit code.");
    return data;
  }
  ...
  async function sendResetEmail(email, userName, code) {
    ...
    try {
      ...
      if (isEmailJsConfigured()) { await sendViaEmailJs(email, userName, code); return; }
      if (localErr.message === "Failed to fetch" || localErr.name === "TypeError") throw new Error("EMAIL_SERVER_OFFLINE");
      throw localErr;
    }
  }
  ...
  async function requestVerificationCode(email, isResend) {
    ...
    try {
      ...
      const devNote = window._lastResetDevMode ? " Code is shown in the start-email.bat window." : "";
      ...
      if (err.message === "EMAIL_SERVER_OFFLINE") {
        setMsg(msgEl, "Email server is not running. Double-click start-email.bat, then try again.", "error");
      } else if (err.message === "FILE_PROTOCOL") {
        setMsg(msgEl, "Open the site at http://localhost:8080 using start-all.bat — not by double-clicking the HTML file.", "error");
      } else {
        setMsg(msgEl, err.message || "Could not send email. Check start-email.bat window for errors.", "error");
      }
      return false;
    } finally {

...
if app_mode == "Detector Agent":
    ...
    if not review_dir.exists() or len(os.listdir(review_dir)) == 0:
        ...
        st.subheader(f"Total Images in Queue: {len(all_images)}")
        
        st.markdown("### Retraining Control Center")
        if st.button("Start Retraining Process", type="primary", use_container_width=True):
            with st.spinner("Retraining the AI model with all updated datasets... Please wait."):
        ...
        for img_name in all_images:
            ...
            with col_actions:
                st.write("Choose action for this image:")
                btn_col1, btn_col2, btn_col3 = st.columns(3)
                
                with btn_col1:
                    if st.button("Confirm as REAL", key=f"real_{img_name}"):
                        target = Path("real") / img_name
                        Path("real").mkdir(exist_ok=True)
                        os.rename(str(img_path), str(target))
                        st.rerun()
                        
                with btn_col2:
                    if st.button("Confirm as AI", key=f"fake_{img_name}"):
                        target = Path("fake") / img_name

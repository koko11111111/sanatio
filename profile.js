...
async function runProfilePage() {
  ...
  // Strict own-profile check — compare lowercased emails
  const isOwn = (me.email.toLowerCase() === targetEmail);

  // Set body attribute so CSS can also guard edit controls
  document.body.setAttribute("data-own-profile", isOwn ? "true" : "false");

  // Firebase already initialized in profile.html inline script
  let db = null;
  try { if (typeof firebase!=="undefined" && firebase.apps.length) db=firebase.firestore(); }
  ...
  // Always start with ALL edit controls hidden — only show for own profile
  ...
  // ── Load user from Firestore then localStorage fallback ────────────────
  ...
  if (db) {
    ...
    } catch(e) { console.warn("Load user from Firestore:", e); }
  }
  // Fallback: localStorage (same device)
  if (!user) {
    try {
      const locals = JSON.parse(localStorage.getItem("aiDetectorUsers")||"[]");
      const found = locals.find(u=>(u.email||"").toLowerCase()===targetEmail);
      if (found) user = found;
    } catch {}
  }
  // Last resort: use logged-in user data if own profile
  if (!user && isOwn) {
    user = { name: me.name, email: me.email, profilePhoto: me.profilePhoto||"", createdAt: me.createdAt||"", bio: "" };
  }

  if (!user) {
    profileNameEl.textContent = "User not found";
    postsFeed.innerHTML = '<p class="fb-empty">This user does not exist or hasn\'t logged in yet.</p>';
    return;
  }
  ...
  // ── Friends count ──────────────────────────────────────────────────────
  let friendsList = [];
  try { friendsList = await FriendSystem.getFriends(targetEmail); } catch(e) { console.warn("getFriends:", e); }
  ...
  if (isOwn) {
    ...
    coverInput.addEventListener("change", async () => {
      ...
      if (!file) return;
      if (file.size>1500000) { alert("Max 1.5 MB"); coverInput.value=""; return; }
      const reader = new FileReader();
      reader.onload = async () => {
        const url = String(reader.result||"");
        coverImg.src = url; coverImg.style.background="";
        if (db) await db.collection("users").doc(userKey).set({coverPhoto:url},{merge:true}).catch(()=>{});
      };
    ...
    avatarInput.addEventListener("change", async () => {
      ...
      if (!file) return;
      if (file.size>800000) { alert("Max 800 KB"); avatarInput.value=""; return; }
      const reader = new FileReader();
      reader.onload = async () => {
        const url = String(reader.result||"");
        avatarEl.src = url;
        if (typeof saveProfilePhotoForCurrentUser==="function") saveProfilePhotoForCurrentUser(url);
        if (db) await db.collection("users").doc(userKey).set({profilePhoto:url},{merge:true}).catch(()=>{});
      };
      reader.readAsDataURL(file);
    });
    ...
    bioForm.addEventListener("submit", async e => {
      ...
      bioForm.classList.add("hidden");
      bioEditBtn.classList.remove("hidden");
      if (db) await db.collection("users").doc(userKey).set({bio},{merge:true}).catch(()=>{});
    });

  // ── OTHER PROFILE: show friend + message buttons only ─────────────────
  } else {
    // Make absolutely sure edit controls stay hidden
    coverEditBtn.classList.add("hidden");
    avatarEdit.classList.add("hidden");
    bioEditBtn.classList.add("hidden");
    bioForm.classList.add("hidden");
  ...
  async function renderFriendsGrid(emails, container, limit) {
    if (!emails || !emails.length) {
      container.innerHTML='<p class="fb-empty">No friends yet.</p>'; return;
    }
    container.innerHTML='<p class="fb-empty" style="font-size:.8rem;color:var(--muted)">Loading…</p>';
    ...
    container.innerHTML=html||'<p class="fb-empty">No friends yet.</p>';
  }

  // ── Load posts from Firestore ──────────────────────────────────────────
  async function loadPosts() {
    if (!db) {
      postsFeed.innerHTML='<p class="fb-empty">Firebase not connected.</p>'; return;
    }
    ...
    const runQuery = async (withOrder) => {
      let q = db.collection("community_posts").where("email","==",targetEmail).limit(30);
    ...
    try {
      ...
      if (!docs.length) {
        postsFeed.innerHTML='<p class="fb-empty">No posts yet.</p>'; return;
      }
    ...
    return `
      <div class="fb-post-card">
        <div class="fb-post-head">
          ...
          <div>
            <a class="fb-post-author" href="profile.html?user=${encodeURIComponent(p.email||"")}">
              ${escHtml(p.author||p.email||"")}
            </a>
  ...
  async function renderFriendButton() {
    ...
    if (status==="friends") {
      ...
      btn.style.display="flex"; btn.style.gap=".5rem";
      btn.innerHTML=`
      ...
      setTimeout(()=>{
        ...
        document.getElementById("fb-decline-btn")?.addEventListener("click", async () => {
          try { await FriendSystem.declineRequest(targetEmail, me.email); await renderFriendButton(); }
          catch(e) { alert(e.message); }
        });
      },0);
    } else {
